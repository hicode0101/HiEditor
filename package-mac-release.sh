...
# 用法: ./package-mac-release.sh [版本号=1.0.0] [排除插件="code txt"] [输出目录=dist]
set -euo pipefail
cd "$(dirname "$0")"
...
cp target/release/HiEditor "$STAGE/"
...
for p in json xml markdown notepad txt code; do
    ...
    if [ -n "$skip" ]; then
        ...
        cp "target/release/libhieditor_$p.dylib" "$STAGE/plugins/$p/bin/hieditor_$p.dylib"
    fi
done
